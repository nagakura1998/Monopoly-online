...
function updateDice() {
	...
	if (document.images) {
		var element0 = document.getElementById("dice0");
		var element1 = document.getElementById("dice1");

		element0.classList.remove("dice-no-img");
		element1.classList.remove("dice-no-img");

		element0.title = "Dice (" + dice0 + " spots)";
		element1.title = "Dice (" + dice1 + " spots)";
		...
		element0.src = "images/Dice_" + dice0 + ".png";
		element0.alt = dice0;
		...
		element1.src = "images/Dice_" + dice1 + ".png";
		element1.alt = dice0;
	} else {
		document.getElementById("dice0").textContent = dice0;
		document.getElementById("dice1").textContent = dice1;

		document.getElementById("dice0").title = "Dice";
		document.getElementById("dice1").title = "Dice";
	}
}
...
function showCardInfo(property) {
	var sq = square[property];
	$("#cardInfo").show();

	$("#cardInfo-normal").hide();
	$("#cardInfo-mortgaged").hide();
	$("#cardInfo-special").hide();

	if (sq.mortgage) {
		$("#cardInfo-mortgaged").show();
		document.getElementById("cardInfo-mortgaged-name").textContent = sq.name;
		document.getElementById("cardInfo-mortgaged-mortgage").textContent = (sq.price / 2);

	} else {

		if (sq.groupNumber >= 3) {
			$("#cardInfo-normal").show();
			document.getElementById("cardInfo-header").style.backgroundColor = sq.color;
			document.getElementById("cardInfo-name").textContent = sq.name;
			document.getElementById("cardInfo-baserent").textContent = sq.baserent;
			document.getElementById("cardInfo-rent1").textContent = sq.rent1;
			document.getElementById("cardInfo-rent2").textContent = sq.rent2;
			document.getElementById("cardInfo-rent3").textContent = sq.rent3;
			document.getElementById("cardInfo-rent4").textContent = sq.rent4;
			document.getElementById("cardInfo-rent5").textContent = sq.rent5;
			document.getElementById("cardInfo-mortgage").textContent = (sq.price / 2);
			document.getElementById("cardInfo-houseprice").textContent = sq.houseprice;
			document.getElementById("cardInfo-hotelprice").textContent = sq.houseprice;

		} else if (sq.groupNumber == 2) {
			$("#cardInfo-special").show();
			document.getElementById("cardInfo-special-name").textContent = sq.name;
			document.getElementById("cardInfo-special-text").innerHTML = utiltext();
			document.getElementById("cardInfo-special-mortgage").textContent = (sq.price / 2);

		} else if (sq.groupNumber == 1) {
			$("#cardInfo-special").show();
			document.getElementById("cardInfo-special-name").textContent = sq.name;
			document.getElementById("cardInfo-special-text").innerHTML = transtext();
			document.getElementById("cardInfo-special-mortgage").textContent = (sq.price / 2);
		}
	}
}
...
window.onload = function() {
	...
	for (var i = 0; i < 40; i++) {
		...
		currentCellPositionHolder.sqId = i;
	...
	$(".cell-position-holder, #jail").on("mouseover", function(){
		...
		// popup("<img src='images/community_chest_icon.png' style='height: 50px; width: 53px; float: left; margin: 8px 8px 8px 0px;' /><div style='font-weight: bold; font-size: 16px; '>Community Chest:</div><div style='text-align: justify;'></div>", function() {
		// 	//communityChestAction(communityChestIndex);
		// });
		//$("#statswrap").show();
		$("#cardswrap").show();
		showCardInfo(this.sqId);
	});

	$("#cardsclose, #cardsbackground").on("click", function() {
		$("#cardswrap").hide();
	});
